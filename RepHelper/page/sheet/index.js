const product_element = '<div class="product-div new-element"><div class="item-description"><div class="product-image-div"><img src="./test.jpg" alt="product_image" class="product-img" width="320px"></div><div class="item-title-div"><span class="item-title">Freakbob (Lorem ipsum odor amet, consectetuer adipiscing elit. Aliquetaliquam fringilla leo imperdiet interdum quis arcu porta nascetur mattis.)</span></div><div class="item-info"><a href="" target="_blank" class="item-link">taobao</a><span class="item-price">21.123.212 - 123.123.213 CNY</span><span class="item-sales">1210 sales</span></div></div><button href="https://google.com/" class="item-view" target="_blank">View Item</button><div class="main-price"></div><div class="low-price"></div><div class="high-price"></div></div>'
let loading = false
...
function addProduct(item) {
    ...
    document.querySelector(".new-element > .item-description > .product-image-div > img.product-img").src = item.images[0] || item.picUrl
...
async function currencies() {
    ...
    prices = (await chrome.storage.local.get(["currency"])).currency
    if (prices === undefined) {
        await chrome.storage.local.set({ "currencyTime": currentTime })
        prices = JSON.parse(await fetch("https://open.er-api.com/v6/latest/CNY"))
        prices = prices.rates
        await chrome.storage.local.set({ "currency": prices })
    }
    const preferred = await chrome.storage.local.get(["preferred"]).preferred
